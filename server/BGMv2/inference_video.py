...
if args.video_target_bgr:
    dataset = ZipDataset(
        [vid, bgr, VideoDataset(args.video_target_bgr, transforms=T.ToTensor())],
...
elif args.video_target_bgr_img:
    dataset = ZipDataset(
        [vid, bgr, [Image.open(args.video_target_bgr_img).convert("RGB")]],
        transforms=A.PairCompose([
            A.PairApply(T.Resize(args.video_resize[::-1]) if args.video_resize else nn.Identity()),
            HomographicAlignment() if args.preprocess_alignment else A.PairApply(nn.Identity()),
            A.PairApply(T.ToTensor())])
...
with torch.no_grad():
    for input_batch in tqdm(DataLoader(dataset, batch_size=1, pin_memory=True)):
        if args.video_target_bgr or args.video_target_bgr_img:
