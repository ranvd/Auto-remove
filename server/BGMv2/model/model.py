...
class MattingBase(Base):
    ...
    """
    # 這裡改 input channel 變成 6 + 1，原本的 Input 加上 MaskRCNN 的輸出。
    def __init__(self, backbone: str):
        super().__init__(backbone, in_channels=6+1, out_channels=(1 + 3 + 1 + 32))
        
    def forward(self, src, bgr, mask):
        x = torch.cat([src, bgr, mask], dim=1)
        x, *shortcuts = self.backbone(x)
        x = self.aspp(x)
        x = self.decoder(x, *shortcuts)
...
class MattingRefine(MattingBase):
    ...
    def forward(self, src, bgr, mask):
        ...
        bgr_sm = F.interpolate(bgr,
                               ...
                               align_corners=False,
                               recompute_scale_factor=True)
        # 我自己加的 mask down sampling
        mask_sm = F.interpolate(mask,
                               scale_factor=self.backbone_scale,
                               mode='bilinear',
                               align_corners=False,
                               recompute_scale_factor=True)
        ...
        # Base
        x = torch.cat([src_sm, bgr_sm, mask_sm], dim=1)
        x, *shortcuts = self.backbone(x)
        x = self.aspp(x)
        x = self.decoder(x, *shortcuts)
